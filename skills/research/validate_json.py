...
def load_fields_yaml(fields_path):
    ...
    # Categories declared by THIS fields.yaml, so the validator is not locked to
    # the hardcoded CATEGORY_MAPPING of the topic it was originally written for.
    declared = {c["category"] for c in data.get("field_categories", []) if c.get("category")}
    category_mapping = {c: [c] for c in declared}
    return all_fields, required_fields, field_categories, category_mapping
...
def validate_json(json_path, all_fields, required_fields, field_categories, category_mapping=None):
    with json_path.open(encoding="utf-8") as f:
        data = json.load(f)
    # Merge the categories this fields.yaml declares with the legacy defaults, so
    # nested category objects are descended into for any research topic.
    merged = dict(CATEGORY_MAPPING)
    for cat, aliases in (category_mapping or {}).items():
        merged.setdefault(cat, list(aliases))
    json_fields = extract_json_fields(data, merged)
...
def main():
    ...
    for json_path in json_files:
        if not json_path.exists():
            ...
            continue
        result = validate_json(json_path, all_fields, required_fields, field_categories, category_mapping)
