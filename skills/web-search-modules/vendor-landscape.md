...
- **Practitioner threads** — Hacker News and topic subreddits on the category, especially migration stories ("we moved off X"). Unfiltered and the fastest route to the failure modes no vendor page lists. Hacker News: `site:news.ycombinator.com`. See `sites/hacker-news.md`. Read the subreddits through the approved Atom-feed listing reader (titles and permalinks, not full threads); pair it with niche forums and Hacker News for the same signal in more depth. See `sites/reddit.md`.
