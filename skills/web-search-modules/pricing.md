...
- **Cross-vendor aggregators** — OpenRouter's model list (per-token prices side by side), Artificial Analysis (price against latency and quality). Ideal for building a comparison set fast, then verify each figure at its source. Verified working: `https://openrouter.ai/models`, `https://artificialanalysis.ai/models`. See `sites/openrouter.md`, `sites/artificial-analysis.md`.
