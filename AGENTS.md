...
- **Planning (the main thread).** Question/answer and exploration, landing on decisions recorded in `PLAN.md`. Makes design calls, owns judgment about whether a module's routing or wording is right. When the shape of something isn't clear yet, that exploration has a structured form: `living-plan`'s `grill` workflow interviews round-by-round until the design tree has been *visited*, so whatever stays undecided lands in Open questions or the Parking lot instead of staying invisible. Ask for it ("grill me", "what am I missing"); offer it in one line — never launch it — when the user is thinking out loud without a question.
...
**Work that belongs to a consumer project stays in that consumer project.** This repo's planning docs track the *package* — skills, agents, modules, the prompts. A local module for one client, a pin bump, where an install lives, or anything else true only of one consuming repo is that repo's business, even when this package is the thing being installed. Tracking it here does not make it more visible; it makes it resurface every session as work that looks queued and cannot be finished from this checkout. When such an item arrives from the mirror, hand it back rather than parking it. The test is simple: if finishing it means editing files outside this repo, it does not belong on this list.
...
That skill both **fills** these docs and **maintains** them: `grill` generates decisions and questions, while `rebalance` and `evict-tasks` relocate what has gone cold. Items ripen along one ladder — parking lot → open question → decision → task queue — and only a grill moves one up it, always propose-then-confirm. Nothing is lost by cutting a grill short; whatever is unfinished is written out as a question or a parked item first.
...
Every module needs the routing header (`Family` / `Use when` / `Do not use for` / `Siblings`), a prioritized source list, and query tactics specific to the domain.

**Sites that recur across modules get their own file at `skills/web-search-modules/sites/<slug>.md`** — `Used by:`, a dated `Reachable:` verdict, `## Query`, `## Worth knowing`. A site earns one by **recurrence** (a second module names it) or by **overflow** (one module's knowledge of it no longer fits under the ~40-line cap), and only once that content actually exists — a file restating the module bullet is an empty container, and two were built and deleted the same day for that reason. Citing modules keep their own self-sufficient access method and add a path citation; a bullet reduced to a bare pointer strands the agent mid-run. A site's reachability verdict lives on its `Reachable:` line, and `ACCESS.md` keeps only what has no site file to live in. The rules an author needs are in `skills/web-search-modules/SKILL.md` and `/research-add-module`'s Steps 4-5. The five upstream modules retain Chinese section headers (`## 搜索源`, `## 查询策略`); new modules should be written in English. `ROADMAP.md` keeps the **Wanted modules** list of domains judged worth adding — check it before inventing a new one. (It is not in `skills/web-search-modules/SKILL.md`; that file deliberately holds operational rules only, and says so.)
