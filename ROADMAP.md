...
| `demand-signals` | Demand signals | Which problems, desires, or frustrations recur in people's own words across independent venues. Niche forums (Discourse JSON search, `site:` fallback for other forum software), a separate `people also ask` pass, autocomplete completion, and review sites, in that order; Amazon Q&A recorded as unreachable rather than silently dropped. Full threads are the wrong unit — listings and titles are the signal, not deep reads. |
...
`demand-signals` opens a family beside `competitor-content` rather than inside it — the two read the same forum layer for opposite reasons, which is why `ROUTING.md` carries a disambiguation paragraph between them rather than a shared row. It is the second module built access-method-first through `/research-add-module`, and the first whose discovery pass probed venue *classes* across several unrelated topics rather than one domain's own source list.
