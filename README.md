...
/research-enumerate      # breadth-first: catalog what exists into catalog.md + outline.yaml
...
/research-harvest        # hand-invoked: tally recurring sources across runs into a candidate list
```
...
19. **`unreachable[]` is a separate output channel from unanswered `uncertain[]`.** A page an agent could not fetch after every escalation available to it, or a domain-constrained search that returned zero results on that domain, is provenance about a source, not a reason to mark a field that was answered through a documented substitute as uncertain. `results/*.json` can carry an `unreachable[]` array of `{source, url, reason}` entries; `/research-report` deduplicates them by that full tuple across every item and renders a reader-visible `## Unreachable sources` section by default — unlike the sources addition below, which renders only on request.

20. **One approved package helper, and a fetch budget that counts every retrieval attempt.** `skills/research/reddit_feed.py` reads Reddit's public Atom listings and is the one script an item agent may invoke by its resolved package path, distinct from writing arbitrary research scripts; its `--max-attempts` option caps its own `429` backoff at the item's remaining fetch budget rather than letting the helper's retries overspend it silently. The fetch budget itself was redefined to count *every* network retrieval attempt — native `WebFetch`, the `crwl`/Firecrawl escalation rungs (see 15 above), and this helper's requests — rather than only native `WebFetch` calls, which is what makes the cap meaningful once more than one route into a page exists.

21. **Source capture and harvest.** Every `results/*.json` can carry a `sources[]` array — `{source, url, fields}` entries naming only the sources that actually supported an answer, recorded in the agent prompt rather than a `fields.yaml` convention that gets forgotten. `/research-harvest` is a hand-invoked skill (never run automatically after `/research-deep` or `/research-report`) that scans every run under the research root, tallies recurring source URLs, and writes `.agents/web-search-modules-local/CANDIDATES.md` — a source is a promotion candidate once it appears in at least 3 runs and supported a field in at least 2 of them. The list is judgment for a human to review, never written straight into a module; entries the human ticks or strikes move to a `## Dispositioned` section and stay excluded from later harvests.

22. **A `demand-signals` module, and a second non-technical family.** None of upstream's five modules, and none of the modules added since, can answer "what do people keep complaining about, in their own words, across places that don't talk to each other" — that question needs raw user language, not a ranked page or a model spec sheet. Built by actually running the discovery pass on seven unrelated probe topics rather than writing a plausible source list: niche forums (Discourse's `search.json` confirmed on two unrelated instances; other forum software falls back to a `site:` query, since XenForo's own community confirms its search is POST-only with no fetchable route), a `people also ask` pass run as its own query, autocomplete completion, and review sites, with Amazon product Q&A recorded as unreachable (503 on every product tested) rather than silently dropped. Its actual rule, not a nicety: full threads are the wrong unit, because repetition across venues is a listing- and title-level signal, and spending `standard`'s 12 fetches opening whole threads buys depth where this module needs breadth instead.

23. **`/research-enumerate` — a breadth-first catalog sweep, for when the hard part is finding the items at all.** The pipeline had exactly one shape of work: `/research` builds an outline from model recall, `/research-deep` investigates each item in depth. Neither answers "what exists in this space" when each item only needs three shallow facts — a link, a one-line description, whatever terms are public — and the real difficulty is enumeration rather than depth per item. `/research-enumerate` sweeps aggregators first (directories, marketplaces, registries, awesome-lists — the thing that lists the leaves, found before the leaves themselves), then one-offs, each phase against its own 20-search/20-fetch ceiling rather than the per-item `quick`/`standard`/`deep` table. Its actual discipline is the coverage declaration: a category the sweep never reached is written down as `Not checked — budget exhausted`, never silently omitted, because an unreached category that goes unmentioned reads as a negative result to every later reader. It writes `catalog.md` as the deliverable and a standard `outline.yaml` beside it, so the run composes with `/research-add-fields` and `/research-deep` with no new wiring — no feeder mode was built, because writing the artifact in the shape the pipeline already reads made one unnecessary.
